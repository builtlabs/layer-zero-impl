...
contract LZEndpointMock is ILayerZeroEndpoint {
    mapping(address => uint16) sendVersion;
    mapping(address => uint16) receiveVersion;
    ...
    mapping(uint16 => mapping(uint16 => mapping(address => mapping(uint => bytes)))) public userApplicationConfig;
    ...
    function getConfig(
        ...
        return userApplicationConfig[_version][_chainId][_ua][_configType];
    }

    function getSendVersion(address _userApplication) external view override returns (uint16) {
        return sendVersion[_userApplication];
    }

    function getReceiveVersion(address _userApplication) external view override returns (uint16) {
        return receiveVersion[_userApplication];
    }

    function setConfig(uint16 _version, uint16 _chainId, uint _configType, bytes memory _config) external override {
        userApplicationConfig[_version][_chainId][msg.sender][_configType] = _config;
    }

    function setSendVersion(uint16 _version) external override {
        sendVersion[msg.sender] = _version;
    }

    function setReceiveVersion(uint16 _version) external override {
        receiveVersion[msg.sender] = _version;
    }

    function mockStoredPayload(uint16 _srcChainId, bytes calldata _path, address dst) external {
        storedPayload[_srcChainId][_path] = StoredPayload(1, dst, bytes32(_path));
    }
